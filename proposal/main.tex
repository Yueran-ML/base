...
The Fourier solution is not the unique generalising circuit.
Zhong et al.\ \cite{zhong2023clock} showed that modular-addition
transformers can discover two qualitatively distinct solution families
(``clock'' and ``pizza''), and Chughtai et al.\ \cite{chughtai2023toy}
extended the analysis to arbitrary finite groups, providing evidence
for a form of universality in how neural networks learn group
operations.
...
empirically.
Kumar et al.\ \cite{kumar2024lazy} frame the same transition as a
move from a lazy (NTK-like) to a rich (feature-learning) regime,
while Lyu et al.\ \cite{lyu2024dichotomy} prove that a dichotomy
between early-phase (kernel) and late-phase (norm-minimising)
implicit biases can provably induce grokking.
Mohamadi et al.\ \cite{mohamadi2024grok} give a margin-based analysis
specifically for modular addition.
These accounts predict that representation geometry should reorganise
during the grokking transition, but do not specify \emph{when} the
reorganisation completes relative to $\taug$---the gap this project
targets.
