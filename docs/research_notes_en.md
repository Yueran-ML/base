...
#### Note on Comprehension classification

Stage 1 classifies lr=1.6e-3 and lr=2.5e-3 at wd=2.5 as Comprehension (tau_gen within the first 33% of training). Stage 0 labeled the same coordinates (grok_B) as Grokking. The discrepancy arises from slightly different `_classify_phase` implementations; the underlying tau_gen values (~10k–13.5k steps) are identical. This does not affect the G<F ordering conclusion.
...
| English research report | **Done ✓** | docs/research_report_en.md |
| Stage 2: wd=2.5 column + boundary multi-seed | Pending | — |
