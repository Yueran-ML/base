...
% ==============================================================
\begin{abstract}
We study temporal dynamics during transformer grokking on modular arithmetic,
directly measuring three distinct milestones in the same training runs:
Fourier circuit formation onset~$\taucirc$ (logit-level),
generalization onset~$\taug$ (test accuracy), and
embedding-geometry consolidation~$\tauf$ (token embedding Fourier alignment).
We provide direct empirical evidence for the \textbf{full three-stage
ordering} $\taucirc < \taug < \tauf$: the Fourier computation circuit forms
before generalization, which in turn precedes the convergence of token
embedding geometry (C$<$G$<$F in 48/54 Grokking runs, 88.9\%).

Specifically, logit Fourier alignment shows circuit formation precedes
generalization in 50/54 runs (92.6\%, median gap 6{,}250~steps),
while embedding geometry lags generalization in 52/55 runs (94.5\%,
median lag $\approx7{,}000$~steps), confirming the G$<$F ordering
of \citeauthor{nanda2023progress}'s ``cleanup'' phase interpretation.
We extend the G$<$F result across four experimental variations:
a dense 7$\times$7 2D (lr$\times$wd) grid (111/130 = 85.4\%),
a different prime $p{=}97$ (21/21 = 100\%, median lag 28{,}000~steps),
and modular multiplication via discrete-log reindexing (30/30 = 100\%).
Together these results establish that embedding-level Fourier geometry
is a \emph{post-generalization structural consequence}---not a
prerequisite---across tasks, primes, and hyperparameter regimes.
\end{abstract}
...
\begin{enumerate}
  \item We introduce a \emph{null-corrected} Fourier alignment metric
    ...
    structure, enabling a principled BIC changepoint estimate of~$\tauf$.
  ...
  \item We extend G$<$F to a dense \textbf{2D grid} of
    (lr$\,\times\,$wd) (Stage~6: 111/130 = 85.4\%), a \textbf{different
    prime} $p{=}97$ (Stage~5B: 21/21 = 100\%, median $\Dtau{=}28{,}000$
    steps), and \textbf{modular multiplication} via discrete-log
    reindexing (Stage~5A: 30/30 = 100\%).

  \item We directly measure \textbf{circuit formation} $\taucirc$ via
    logit Fourier alignment in the same training runs (Stage~4),
    finding that $\taucirc < \taug$ in 50/54 Grokking runs (92.6\%,
    median gap 6{,}250 steps) and establishing the \emph{full} three-stage
    ordering C$<$G$<$F in 48/54 runs (88.9\%).
  ...
  \item We show that for cells with $|\Delta\tau|\ge3{,}000$ steps,
    ...
    positives near the coincident boundary (Table~\ref{tab:raw_corr},
...
as a first-order phase transition, while \citet{lyu2024dichotomy}
prove that a dichotomy of early- and late-phase implicit biases
can provably induce grokking in homogeneous networks.
These works characterise \emph{when} generalization occurs; our
work characterises its timing \emph{relative} to embedding
geometry and circuit formation.
...
Our $\tauf$ result is analogous: the Fourier ring geometry of
token embeddings is also a \emph{post-generalization} geometric
consolidation, not a prerequisite for generalization.
...
In 50/54 Grokking runs (92.6\%), $\taucirc < \taug$.
...
The dominant pattern is \textbf{C$<$G$<$F} in 48/54 runs (88.9\%).
...
\textbf{30/30 (100\%) show G$<$F ordering} after discrete-log
reindexing of token embeddings.
...
\textbf{21/21 Grokking runs (100\%) show G$<$F ordering.}
...
\begin{table}[t]
  ...
  \begin{tabular}{llccc}
    \toprule
    Stage & Setting & $n_G$ & G$<$F rate & med.\ $\Dtau$ (k) \\
    \midrule
    2+3  & add mod 53, 1D sweeps & 55  & 94.5\% & $7.0$ \\
    5A   & mul mod 53            & 30  & \textbf{100\%}  & $8.8$ \\
    5B   & add mod 97            & 21  & \textbf{100\%}  & $28.0$ \\
    6    & add mod 53, 2D grid   & 130 & 85.4\% & $6.0$ \\
    \midrule
    \textbf{All} & & \textbf{236} & \textbf{89.8\%} & --- \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{111/130 Grokking runs (85.4\%) show G$<$F ordering};
14/130 show F$<$G and 5/130 have G\_only ($\tauf$ not detected).
...
\paragraph{Phase-boundary regime and task-specificity.}
Both F$<$G runs in our addition sweeps occur at the margins of the
Grokking phase where $\taug$ is unusually late
(43{,}500 and 67{,}500 steps).
A pilot run of modular \emph{subtraction} ($(a{-}b)\bmod 53$, wd$=2.18$)
further supports this pattern: subtraction groks much later than addition
at the same hyperparameters ($\taug{=}43{,}500$ vs $\approx11{,}000$
for addition), and the ordering is F$<$G ($\Dtau{=}{-}9{,}000$).
Together, these observations suggest a \textbf{speed-dependent ordering
hypothesis}: G$<$F holds when generalization is fast (small $\taug$);
when grokking is delayed---whether by extreme hyperparameters or by a
harder task---weight-decay-driven embedding consolidation can precede
generalization, yielding F$<$G.
Formally testing this hypothesis across a wider task and hyperparameter
space is an important open direction.
Figure~\ref{fig:speed_ordering} provides preliminary quantitative
support: across 57 Grokking runs (55~addition + 2~subtraction pilots),
$\Dtau$ correlates negatively with $\taug$ ($r=-0.62$); all F$<$G
runs have $\taug \geq 43{,}000$ steps, whereas the majority of G$<$F
runs have $\taug \leq 35{,}000$ steps.
Crucially, two subtraction pilots at \emph{the same hyperparameters}
(wd$=2.18$, lr$=1.6\!\times\!10^{-3}$) provide within-task evidence:
seed~42 ($\taug{=}43{,}500$, slow) gives F$<$G ($\Dtau{=}-9{,}000$),
while seed~7 ($\taug{=}27{,}500$, faster) gives G$<$F
($\Dtau{=}+5{,}500$)---the same hyperparameters produce opposite
orderings depending solely on how quickly the run generalizes.
We note that $\Dtau = \tauf - \taug$ is partly mathematically related
to $\taug$, so the aggregate correlation partly reflects this
construction; however, the within-task seed comparison is free of this
confound and directly supports the speed-dependent interpretation.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.80\linewidth]{speed_ordering_scatter.png}
  \caption{%
    \textbf{Speed-dependent ordering hypothesis.}
    Scatter of generalization onset $\taug$ vs lag $\Dtau{=}\tauf{-}\taug$
    for all 55 Grokking addition runs (blue circles = G$<$F, red stars
    = F$<$G, grey diamond = coincident) and 2 subtraction pilots (orange
    triangles; 1 F$<$G, 1 G$<$F).
    All F$<$G runs cluster at $\taug\geq43{,}000$ steps; the
    overall correlation is $r{=}{-}0.62$ ($n{=}57$).
    Note: since $\Dtau$ is defined relative to $\taug$, part of this
    negative correlation is a mathematical artifact; the result should
    be interpreted as suggestive evidence rather than a clean causal claim.
  }
